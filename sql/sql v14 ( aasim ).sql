...
insert  into `subject`(`id`,`name`,`credit_hour`,`course_code`,`sem_id`,`sub_type`) values 
(1,'ITC',3,'CS-1',4,0),
(2,'PF',3,'CS-1',4,0),
(3,'PF',1,'CS-1',4,1),
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`mohsin`@`%` SQL SECURITY DEFINER VIEW `getmarks` AS select `ml`.`id` AS `id`,`std`.`rollno` AS `rollno`,`std`.`fullname` AS `Name`,`sub`.`name` AS `Subject`,(case when (`sub`.`sub_type` = 0) then 'TH' else 'PR' end) AS `Sub_Type`,`ml`.`mark` AS `mark`,(case when (`sub`.`sub_type` = 0) then `FINDGP`(`ml`.`mark`,100) else `FINDGP`(`ml`.`mark`,50) end) AS `GP`,(case when (`sub`.`sub_type` = 0) then `FINDGRADE`(`ml`.`mark`,100) else `FINDGRADE`(`ml`.`mark`,50) end) AS `GRADE`,(case when (`sub`.`sub_type` = 0) then `FINDREMARKS`(`ml`.`mark`,100) else `FINDREMARKS`(`ml`.`mark`,50) end) AS `Remarks` from ((`marks_ledger` `ml` join `subject` `sub` on((`sub`.`id` = `ml`.`sub_id`))) join `student` `std` on((`std`.`id` = `ml`.`std_id`))) */;
