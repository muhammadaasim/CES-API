...
(1,'Saleem','Shaikh','23412412342','saleem','saleem','12/12/2012',0),
(2,'Shahid Ali','Shahid Ali','12412342343423','shahid','shahid','1618857217181',0),
(3,'kujh be','kujh be','12343-2134312-1','kujh be','kujhbe','1618858435090',0);
...
(2,'Parkash Lal','Bhangani',2,'parkash','parkash',NULL,'http://localhost:4000/uploads/download.jpg'),
(3,'Rajpar','Rajpar',4,'rajpar','rajpar','124123412322423','http://localhost:4000/uploads/start-0e837dcc57769db2306d8d659f53555feb500b3c5d456879b9c843d1872e7baa.jpg');
...
(2,'Muhammad Ali','Muhammad Ali','m.ali','m.ali'),
(3,'Shoaikh Ali','Shoaikh Ali','shoaib','shoain');
...
(5,2,2021,2,0,3,NULL,'1618899306036','http://localhost:4000/uploads/download.jpg'),
(6,1,2021,2,0,3,NULL,'1618899773927','http://localhost:4000/uploads/download.jpg'),
(7,2,2021,3,0,0,'','1618905885674','http://localhost:4000/uploads/download.jpg');
...
(10,3,2021,1,'1618899368717',5),
(11,3,2021,1,'1618899374876',6);
...
(1,1,2,2,1,18,4,'1618653921511',8,17,43,'Approved By Controller'),
(2,1,2,2,1,7,4,'1618653982575',19,19,45,'Approved By Controller'),
(3,2,5,1,2,15,4,'1618654518034',9,18,42,'Approved By Controller'),
(4,1,5,1,2,10,4,'1618655199222',6,11,27,'Approved By Controller'),
(5,2,5,1,2,10,4,'1618655328381',9,13,32,'Approved By Controller'),
(6,1,5,1,2,7,4,'1618655386477',7,8,22,'Approved By Controller'),
(7,1,5,1,2,11,4,'1618655490801',8,10,29,'Approved By Controller'),
(8,1,5,1,1,11,4,'1618655570174',8,10,29,'Approved By Controller'),
(9,1,6,1,1,30,4,'1618655589185',20,40,90,'Approved By Controller'),
(10,2,1,2,4,0,4,'1618893227440',0,20,20,'Approved By Controller'),
(11,2,2,2,4,0,4,'1618893235382',0,24,24,'Approved By Controller'),
(12,2,1,2,3,0,4,'1618893246545',0,44,44,'Approved By Controller'),
(13,2,2,2,3,27,4,'1618893263575',17,45,89,'Approved By Controller'),
(14,2,2,2,2,29,4,'1618893298027',20,23,72,'Approved By Controller'),
(15,2,1,2,2,17,4,'1618893326343',20,24,61,'Approved By Controller'),
(16,2,10,3,5,22,4,'1618899642300',20,24,66,'Approved By Controller'),
(17,2,11,3,5,0,4,'1618899691070',0,22,22,'Approved By Controller'),
(18,1,10,3,6,23,4,'1618899912664',17,36,76,'Approved By Controller'),
(19,1,11,3,6,0,4,'1618899922090',0,42,42,'Approved By Controller');
...
(3,'BS-BBA-005','Priya',3,'Tejwani','1999-02-16','Hindhu',2,'Pakistan','priya','priya','Shikarpur','Shikarpur','Khabar nahe','1618824011138',0,'http://localhost:4000/uploads/download.jpg');
...
(2,'ITC (PR)',1,'BSIT-1',1,1,0),
(3,'Pak Study',3,'BSIT-1',1,0,0),
(4,'Elementry Math',3,'BSIT-1',1,0,0),
(5,'PF (TH)',3,'BSIT-1',1,0,1),
(6,'PF (PR)',1,'BSIT-1',1,1,1),
(7,'FYP',3,'BSIT-8',8,1,0),
(8,'Web Services',3,'BSIT-8',8,0,0),
(9,'Organization Behaviour',3,'Com-1',17,0,0),
(10,'OOP',3,'BSIT-2',2,0,1),
(11,'OOP',1,'BSIT-2',2,1,1),
(12,'Differential equation',2,'BSIT-4',4,0,0);
...
(3,'kashif','dayo',1,'kashif','kshif',NULL,NULL,0,'http://localhost:4000/uploads/download.jpg'),
(4,'Farhan Ali','Surahio',1,'Farhan','farhan123',NULL,NULL,0,'http://localhost:4000/uploads/download.jpg');
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`mohsin`@`%` SQL SECURITY DEFINER VIEW `getsubjmarks` AS select `std`.`id` AS `std_id`,`std`.`rollno` AS `rollno`,`std`.`gender` AS `gender`,`std`.`fathername` AS `fathername`,`sm`.`id` AS `sem_id`,`sm`.`name` AS `semester`,`p`.`id` AS `prog_id`,`p`.`name` AS `program`,`p`.`fullname` AS `programfullname`,`d`.`id` AS `dept_id`,`d`.`name` AS `dept`,`ef`.`id` AS `form_id`,`sb`.`id` AS `subj_id`,`sb`.`name` AS `subj_name`,`sb`.`type` AS `sub_type`,`sb`.`credit_hour` AS `credit_hour`,`sb`.`course_code` AS `course_code`,`ml`.`totalmark` AS `total_marks` from ((((((`marks_ledger` `ml` join `student` `std` on((`std`.`id` = `ml`.`std_id`))) join `subject` `sb` on((`sb`.`id` = `ml`.`sub_id`))) join `program` `p` on((`p`.`id` = `std`.`prog_id`))) join `dept` `d` on((`p`.`dept_id` = `d`.`id`))) join `semester` `sm` on((`sm`.`id` = `sb`.`sem_id`))) join `examform` `ef` on((`ef`.`id` = `ml`.`f_id`))) */;
