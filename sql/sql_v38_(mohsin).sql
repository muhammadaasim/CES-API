...
(1,'PF (TH)',3,'BSIT-1',9,0,0),
(2,' PF (PR)',1,'BSIT-1',9,1,0),
(3,'Elementry Math',2,'BSIT-3',9,0,0),
(4,'Pak Study',3,'BSIT-1',9,0,0),
(5,'English',3,'BSIT-1',9,0,0);
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `getsubjbydept` AS select `sub`.`id` AS `id`,`sub`.`name` AS `Subj_Name`,`sm`.`id` AS `Sem_id`,`sm`.`name` AS `Sem_Name`,`p`.`id` AS `program_id`,`p`.`name` AS `program_name`,`p`.`dept_id` AS `dept_id` from ((`subject` `sub` join `semester` `sm` on((`sm`.`id` = `sub`.`sem_id`))) join `program` `p` on((`p`.`id` = `sm`.`prog_id`))) */;
