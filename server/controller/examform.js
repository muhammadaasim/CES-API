...
module.exports.getExamforms = (req, res) => {
	try {
		connection.getConnection((err, connection) => {
			...
			connection.query('SELECT * FROM getexamformdata', function(err, rows, fields) {
				if (err) throw err;
				console.log(rows);
...
module.exports.getExamFormByDeptId = (req, res) => {
	try {
		connection.getConnection((err, connection) => {
			if (err) throw err;
			connection.query('SELECT * FROM getexamformdata WHERE dept_id=?', [ req.params.id ], function(
				err,
				rows,
				fields
			) {
...
module.exports.getExamformswithsubj = (req, res) => {
	try {
		connection.getConnection((err, connection) => {
			if (err) throw err;
			console.log('MySQL Connection Established: ', connection.threadId);
			connection.query('SELECT * FROM getexamforms ', function(err, rows, fields) {
				if (err) throw err;
				console.log(rows);
				res.send({ error: '', success: 'success', result: rows });
				connection.release((err) => console.log(err));
...
module.exports.GetSlip = (req, res) => {
	try {
		connection.getConnection((err, connection) => {
			...
			console.log('MySQL Connection Established: ', connection.threadId);
			connection.query(
				'SELECT * FROM GETSLIP WHERE session = ? AND std_id = ?',
				[ req.params.session, req.params.id ],
				function(err, rows, fields) {
					if (err) throw err;
					console.log(rows);
					res.send({ error: '', success: 'success', result: rows });
...
module.exports.getExamFormwithsubByStdId = (req, res) => {
	try {
		connection.getConnection((err, connection) => {
			if (err) throw err;
			connection.query('SELECT * FROM getexamforms WHERE student_id=?', [ req.params.id ], function(
				err,
				rows,
				fields
			) {
